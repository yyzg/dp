...
local function on_frida_call(arg1, arg2, arg3)
    ...
    -- frida(js) -> lua 通讯建议使用json, 避免逗号分隔导致内容截断
    if type(arg3) == "string" and #arg3 > 0 and string.sub(arg3, 1, 1) == "{" then
        local ok, payload = pcall(json.decode, arg3)
        if ok and type(payload) == "table" then
            if payload.cmd == "log" then
                local level = tostring(payload.level or "info")
                local msg = tostring(payload.msg or "")
                if level == "error" then
                    logger.error("[frida] %s", msg)
                elseif level == "warn" then
                    logger.warn("[frida] %s", msg)
            ...
            logger.warn("frida payload json decode fail, ok=%s", tostring(ok))
        end
    end
...
local ret = frida.load("DP2 load success!", on_frida_call)
logger.info("frida.load ret=%d", ret)


local function mainDpLoad(_user)
    game.fac.user(_user):SendNotiPacketMessage(
        decode_unicode('\\u4eca\\u5929, \\u4e5f\\u662f\\u5145\\u6ee1\\u5e0c\\u671b\\u7684\\u4e00\\u5929!'),
         14)
end
dpx.hook(game.HookType.Reach_GameWord, mainDpLoad)

local function onDungeonAfterLoading(_user)
    local user = game.fac.user(_user)
    local userName = user:GetCharacName()
    local dungeon = user:GetDungeon()
    local dungeonName = dungeon:GetName()
    user:SendNotiPacketMessage(string.format("副本[%s]开始", dungeonName))
end
dpx.hook(game.HookType.DungeonAfterLoading, onDungeonAfterLoading)
...
dpx.set_auction_min_level(60) -- 设置等级上限
